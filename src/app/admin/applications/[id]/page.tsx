...
function txDisplayId(tx: TransactionRow): string {
...
export default function AdminApplicationViewPage() {
  ...
  async function handleDeleteUser() {
    ...
    try {
      const ok = await runDeleteUserFlow({
        applicationId: app.id,
        mobile: app.user?.mobile ?? app.mobileNumber,
      });
      if (!ok) return;
      setTx([]);
      await load();
  ...
  const deleteAccountMobile = app?.user?.mobile ?? app?.mobileNumber ?? null;
        ...
        actions={
          <>
            ...
            <button
              ...
              className="rounded-lg border border-red-300 bg-red-50 px-3.5 py-2 text-sm font-semibold text-red-700 shadow-sm transition hover:bg-red-100 disabled:opacity-50"
            >
              Delete user
            </button>
          </>
                ...
                <div className="grid gap-3">
                  ...
                  <div className="rounded-xl border border-red-200 bg-red-50/60 px-4 py-3">
                    ...
                    <p className="mt-1 text-sm text-red-900/90">
                      Permanently removes the login account
                      {deleteAccountMobile ? ` (${deleteAccountMobile})` : ""}, all
          ...
          >
            ...
            <div className="space-y-4 px-5 py-5">
              ...
              <div className="space-y-2">
                ...
                <input
                  disabled
                  value={(openCommissionTx.note as string | null | undefined) ?? ""}
                  readOnly
                  className="min-h-11 w-full rounded-lg border border-zinc-200 bg-white px-3 text-sm text-zinc-700 outline-none disabled:cursor-not-allowed"
                />
              </div>

              {openCommissionTx.status === "PENDING" ? (
                <div className="grid grid-cols-2 gap-3 pt-2">
                  <button
                    type="button"
                    disabled={txActionId === openCommissionTx.id}
                    onClick={() => setTransactionStatus(openCommissionTx.id, "APPROVED")}
                  ...
                  <button
                    type="button"
                    disabled={txActionId === openCommissionTx.id}
                    onClick={() => setTransactionStatus(openCommissionTx.id, "DECLINED")}
